...
export default class ImageFinder {
  ...
  static async getImageByNameWidthAndHeight(obj:ImageRequestData): Promise<string>{

    const baseImagePath: string = this.getBaseImagePath(obj);
    const thumbImagePath: string = this.getThumbImagePath(obj);
    const thumbAccess:string = (thumbImagePath) ? await this.checkImageAccessByPath(thumbImagePath): '';
    
    if (thumbAccess){
      return thumbImagePath;
    }
    
    const baseImageAccess:string = await this.checkImageAccessByPath(baseImagePath);
    if (!baseImageAccess){
      return "";
    }
    const resizedImagePath:string = await ImageResizer.resizeImage(obj, baseImagePath, thumbImagePath);
  ...
  static buildPath(basePath: string, fileName: string): string {
    return basePath && fileName ? path.resolve(__dirname, "../..", basePath, fileName) : '';
  }
