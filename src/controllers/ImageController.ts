...
export default class ImageController {
  ...
  static validateRequest(obj: ImageRequestData): boolean {
    return Boolean(
      obj.filename && obj.width && obj.height && this.isPositiveInteger(obj)
    );
  }
  ...
  static sendInvalidURLRequest(res: Response): void {
    res
      .status(400)
      .send(
        'Invalid parameters. Please provide name, with and height. Width and height should be non negative.'
      );
  }
